...
class RegisterController {
  ...
  Future<void> init(BuildContext context, Function refresh) async {
    ...
    _progressDialog = ProgressDialog(context: context); // Inicialización correcta
  }
  ...
  void register() async {
    ...
    Stream? stream = await usersProvider.createWithImage(user, imageFile!); // Asegúrate de que no sea nulo
    stream?.listen((res) {
  ...
  Future selectImage(ImageSource imageSource) async {
    pickedFile = await ImagePicker().pickImage(source: imageSource); // Cambiado a `pickImage`
    if (pickedFile != null) {
      imageFile = File(pickedFile!.path); // Asegura que no es nulo
    }
